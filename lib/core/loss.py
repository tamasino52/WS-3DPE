...
class LimbLengthLoss(nn.Module):
    ...
    def forward(self, output, target, mean_length, target_weight):
        batch_size = output.size(0)
        num_joints = output.size(1)
        ...
        for idx in range(num_joints):
            ...
            loss += target_weight[num_joints] * self.mse(pose_length, mean_length)
...
class ScaleRegularization(nn.Module):
    def __init__(self, mean_length):
        super(ScaleRegularization, self).__init__()
        self.mean_length = mean_length
        self.mse = nn.MSELoss(reduction='mean')

    def forward(self, output, target, mean_length):
        batch_size = output.size(0)
        num_joints = output.size(1)
        pose_output = output.reshape((batch_size, num_joints, -1)).split(1, 1)
        pose_target = target.reshape((batch_size, num_joints, -1)).split(1, 1)
        ...
        for idx in range(num_joints):
            ...
            loss.append(torch.pow((pose_length - mean_length), 2))
